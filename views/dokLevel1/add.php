                    ...
                    <div class="row">
                        <select name="jenis_dokumen" class="easyui-combobox" label="Jenis Dokumen" 
                            ...
                            data-options="required:true,prompt:'Select Type',value:'',panelHeight:'auto'" >
                                <option value="Pedoman Mutu">Pedoman Mutu</option>
                                <option value="Site Master File">Site Master File</option>
                                <option value="Pedoman Halal">Pedoman Halal</option>
                        </select>
                    </div>
                    <div class="row">
...
<script type="text/javascript">
    function add_dokumen() {
        ...
        $('#trxpr_form').form('submit', {
            url: '<?php echo $this->createUrl('/dokLevel1/create'); ?>',
            onSubmit: function () {
            ...
            success: function (result) {
                ...
                if (result.success) {
                    jQuery('#dg-doklevel1').datagrid('reload');
                    //var currTab = $('#main-tabs').tabs('getSelected');
                    //var tabIdx = $('#main-tabs').tabs('getTabIndex', currTab);
                    //$('#main-tabs').tabs('close', tabIdx);
                    if ( $('#' + result.data.no_dokumen ).length ) {
                        $('#dok_level1').append(
                            '<li id="' + result.data.no_dokumen + '"><span><a href="javascript:void(0)" ' +
                                'onclick="open_tabs(\'/dokumenmutu/site/displayDokLvl1/jenisdok/' +
                                result.data.jenis_dokumen + '/nodok/' + result.data.no_dokumen + '\',\'' + 
                                result.data.no_dokumen + '\')">' + result.data.no_dokumen + '</a></span>' +
                            '</li>');
                    } else {
                        $('#' + result.data.no_dokumen).text(result.data.no_dokumen);
                    }
