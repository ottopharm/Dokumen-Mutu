...
class DokLevel2Controller extends Controller {
    ...
    public function search($dept) {
        ...
        $data_count = Yii::app()->db->createCommand("{CALL QA_SearchDokLevel2All(:DeptID, :JudulDokumen)}")
                ->bindParam(':DeptID', $dept_id, PDO::PARAM_STR)
                ->bindParam(':JudulDokumen', $judul_dok, PDO::PARAM_STR)
                ->queryScalar();
        ...
        //-- Stored Procedure di bawah ini hanya berlaku untuk Ms. SQL
        $data = Yii::app()->db->createCommand("{CALL QA_SearchDokLevel2(:DeptID, :JudulDokumen, :Offset, :Rows)}")
                ->bindParam(':DeptID', $dept_id, PDO::PARAM_STR)
                ->bindParam(':JudulDokumen', $judul_dok, PDO::PARAM_STR)
                ->bindParam(':Offset', $start_row, PDO::PARAM_INT)
                ->bindParam(':Rows', $end_row, PDO::PARAM_INT)
                ->queryAll();

        foreach ($data as $item) {
            $row[] = array(
                'id' => $item['NoDokumen'],
                'judul_dok' => $item['JudulDokumen'],
                'tgl_upload' => date('d-M-Y', strtotime($item['TglUpload'])),
                'dept_id' => $item['DeptID'],
